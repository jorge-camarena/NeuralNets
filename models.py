...
class DigitClassificationModel(object):
    ...
    def __init__(self):
        ...
        self.w0 = nn.Parameter(784, 60)
        self.b0 = nn.Parameter(1, 60)
        self.w1 = nn.Parameter(60, 10)
    ...
    def train(self, dataset):
        ...
        while currAcc < 0.972:
            for x, y in dataset.iterate_once(100):
                ...
                for p in range(len(params)):
                    params[p].update(gradients[p], -0.1)
                currAcc = dataset.get_validation_accuracy()
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        numLanguages = len(self.languages)

        # Initialize your model parameters here
        self.w0 = nn.Parameter(self.num_chars, 128)
        self.b0 = nn.Parameter(1, 128)
        self.hiddenW = nn.Parameter(128, 128)
        self.hiddenB = nn.Parameter(1, 128)
        self.outputW = nn.Parameter(128, numLanguages)
        self.outputB = nn.Parameter(1, numLanguages)
    ...
    def run(self, xs):
        ...
        Returns:
            A node with shape (batch_size x 5) containing predicted scores
                (also called logits)
        """
        firstLayer = nn.AddBias(nn.Linear(xs[0], self.w0), self.b0)

        for c in xs[1:]:
            dotProd1 = nn.Linear(c, self.w0)
            added = nn.Add(dotProd1, firstLayer)
            dotProd2 = nn.Linear(added, self.hiddenW)
            withBias = nn.AddBias(dotProd2, self.hiddenB)
            firstLayer = nn.ReLU(withBias)

        lastLayer = nn.AddBias(nn.Linear(firstLayer, self.outputW), self.outputB)
        return lastLayer
    ...
    def train(self, dataset):
        ...
        """
        currAcc = 0.0
        params = [self.w0, self.b0, self.hiddenW, self.hiddenB, self.outputW, self.outputB]
